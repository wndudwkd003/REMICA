...
MAX_TRAIN_SAMPLES: int | None = 3000  # train split 최대 샘플 수
MIN_EACH_SPLIT = 1  # valid/test 최소 몇 개는 남길지

os.makedirs(OUT_DIR, exist_ok=True)


def limit_max_samples(
    results: dict,
    max_train: int | None,
    seed: int,
    ...
    - 최종적으로 train : valid : test 비율을 ratio로 맞춤
    - 기준은 max_train (train 상한). valid/test는 ratio에 맞춰 자동 결정
    - 각 split 내부에서는 label(0/1) 균형 유지하며 샘플링
    """
    if max_train is None:
        return results

    tr_r, va_r, te_r = ratio
    if tr_r <= 0 or va_r < 0 or te_r < 0:
        raise ValueError(
            f"Invalid ratio={ratio}. train ratio must be >0, valid/test must be >=0."
        )
    ...
    # 1) train 타겟 개수 (상한)
    target_train = min(n_train, int(max_train))

    # 2) ratio 기준으로 valid/test 타겟 계산 (train 대비 비율)
    #    valid = train * (va_r / tr_r), test = train * (te_r / tr_r)
    #    round로 근사 (원하면 floor/ceil로 바꿔도 됨)
    target_valid = int(round(target_train * (va_r / tr_r))) if va_r > 0 else 0
    target_test = int(round(target_train * (te_r / tr_r))) if te_r > 0 else 0

    # 최소 개수 보장(비율이 0이면 0 유지)
    if va_r > 0:
        target_valid = max(target_valid, min_each_split)
    if te_r > 0:
        target_test = max(target_test, min_each_split)

    # 원본 개수보다 클 수 없으므로 clip
    target_valid = min(n_valid, target_valid)
    target_test = min(n_test, target_test)
...
def get_toxispanse(dataset_path: Path, dataset_name: str):
    ...
    for _, row in df.iterrows():
        base = _norm_id(row.get("id", None))
        if not base:
            base = f"missingid_{int(row['_orig_row'])}"

        uid = base
        # 혹시라도 중복이면 원본 row 번호로 강제 유니크화
        if uid in seen:
            uid = f"{base}__{int(row['_orig_row'])}"
        seen.add(uid)
        uids.append(uid)
    ...
    def to_items(df_split, split_name: str):
        ...
        for _, row in df_split.iterrows():
            ...
            items.append(
                {
                    # ✅ row['id']가 아니라 row['uid'] 사용
                    "id": f"{dataset_name}_{split_name}_{row['uid']}",
                    "text": text,
                    "label": int(row["label"]),
                    "metadata": {
                        ...
                        # 디버깅용으로 원본 id도 남기고 싶으면:
                        # "raw_id": None if pd.isna(row.get("id")) else str(row.get("id")),
...
def draw_label_distribution(results, output_dir: Path):
    ...
    for i, label in enumerate(labels):
        ...
        # ===== 막대 위에 숫자 표시 =====
        try:
            ax.bar_label(b, labels=[str(v) for v in values], padding=3, fontsize=10)
        except Exception:
            # matplotlib 구버전 fallback
    ...
    # ===== split별 총합도 같이 표시(선택) =====
    totals = [sum(counts[s].get(l, 0) for l in labels) for s in splits]
    for i, total in enumerate(totals):
        ax.text(
            x[i] + bar_width / 2,
            max(counts[splits[i]].get(0, 0), counts[splits[i]].get(1, 0)) * 1.05 + 0.5,
            f"n={total}",
            ha="center",
            va="bottom",
            fontsize=10,
...
def do_check_count(results):
    ...
    if MAX_TRAIN_SAMPLES is not None:
        results = limit_max_samples(
            results,
            MAX_TRAIN_SAMPLES,
            seed=SEED,
            ratio=SPLIT_RATIO,
            min_each_split=MIN_EACH_SPLIT,
        )

        print(
            f"== After MAX_TRAIN_SAMPLES={MAX_TRAIN_SAMPLES} with ratio={SPLIT_RATIO} =="
        )
